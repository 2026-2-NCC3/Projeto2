...
export default function Navbar({ activeItem, darkMode, onToggleDark, onLogout, usuario }: NavbarProps) {
  ...
  return (
    <header className="h-16 bg-white border-b border-gray-200 flex items-center px-6 gap-4 flex-shrink-0 relative z-20">
      {/* Breadcrumb */}
          ...
          {notifOpen && (
            <div className="absolute right-0 top-full mt-2 w-80 bg-white border border-gray-200 rounded-xl shadow-lg overflow-hidden">
              ...
              <div className="divide-y divide-gray-50">
                {NOTIFICATIONS.map(n => (
                  <div key={n.id} className={`px-4 py-3 hover:bg-gray-50 cursor-pointer ${n.unread ? 'bg-blue-50/30' : ''}`}>
        ...
        <div className="relative">
          ...
          {profileOpen && (
            <div className="absolute right-0 top-full mt-2 w-48 bg-white border border-gray-200 rounded-xl shadow-lg overflow-hidden">
              ...
              <div className="p-1">
                {['Meu perfil', 'Configurações'].map(item => (
                  <button key={item} onClick={closeAll} className="w-full text-left px-3 py-2 text-sm text-gray-700 hover:bg-gray-100 rounded-lg transition-colors">
                    {item}
                  </button>
                ))}
                <button
