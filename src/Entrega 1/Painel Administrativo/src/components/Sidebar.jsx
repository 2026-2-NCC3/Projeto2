...
import { LayoutDashboard, Users, BookOpen, Calendar, Award, Brain, MessageSquare, BarChart3, Settings, Shield, FileText, ChevronDown, ChevronRight, LogOut, Menu, X, University, CheckSquare, Bell } from 'lucide-react';
const NAV_ITEMS = [
...
export default function Sidebar({ activeItem, onNavigate, collapsed, onToggle, onLogout, usuario }) {
    ...
    const isActive = (item) => {
        if (item.id === activeItem)
            return true;
        if (item.children)
            return item.children.some(c => c.id === activeItem);
        return false;
    };
    const renderItem = (item, depth = 0) => {
        ...
        <button onClick={() => hasChildren ? toggleGroup(item.id) : onNavigate(item.id)} title={collapsed ? item.label : undefined} className={`
        ...
        {hasChildren && !collapsed && isExpanded && (<div className="mt-0.5 mb-0.5">
    ...
    return (<aside className={`flex flex-col h-screen bg-white border-r border-gray-200 transition-all duration-200 flex-shrink-0 ${collapsed ? 'w-[72px]' : 'w-[260px]'}`}>
      {/* Header */}
      ...
      <div className={`border-t border-gray-200 p-3 ${collapsed ? 'flex justify-center' : ''}`}>
        {collapsed ? (<button onClick={onLogout} title="Sair" className="p-1.5 rounded-md hover:bg-gray-100 text-gray-500 hover:text-red-500 transition-colors">
            <LogOut size={18}/>
