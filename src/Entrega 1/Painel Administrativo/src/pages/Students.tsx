...
interface DetailProps { student: Student; onClose: () => void }

function StudentDetail({ student, onClose }: DetailProps) {
        ...
        <div className="p-6">
          ...
          <div className="flex border-b border-gray-200 mb-4 overflow-x-auto">
            {tabs.map(t => (
              <button key={t} onClick={() => setTab(t)}
                className={`px-4 py-2 text-sm whitespace-nowrap border-b-2 transition-colors ${tab === t ? 'border-blue-600 text-blue-700 font-medium' : 'border-transparent text-gray-500 hover:text-gray-700'}`}>
                {t}
              </button>
            ))}
          </div>
          {tab === 'Visão geral' && (
            <div className="space-y-4">
              {([['Nome completo', student.name], ['E-mail', student.email], ['Status', student.status], ['Cadastro', student.joined], ['Cursos', student.courses.join(', ')]] as [string, string][]).map(([label, value]) => (
          ...
          {tab === 'Cursos' && (
            <div className="space-y-3">
              {student.courses.map(c => (
                <div key={c} className="p-3 border border-gray-200 rounded-lg">
          ...
          {tab !== 'Visão geral' && tab !== 'Cursos' && (
            <div className="py-12 text-center text-gray-400 text-sm">Dados de {tab.toLowerCase()} em desenvolvimento</div>
          )}
        </div>
      </div>
    </div>
...
// ── Student form (new / edit) ──────────────────────────────────────────────

interface FormData { name: string; email: string; courses: string[]; lgpd: boolean }
const emptyForm = (): FormData => ({ name: '', email: '', courses: [], lgpd: false });
...
function StudentForm({ initial, onSave, onClose }: StudentFormProps) {
  ...
  const validate = () => {
    ...
    if (!form.lgpd) e.lgpd = 'Consentimento LGPD obrigatório para cadastro';
    return e;
  };
          ...
          <div className="p-6 space-y-4">
            ...
            <div>
              ...
              <input
                ...
                className={`w-full px-3 py-2 border rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 ${errors.name ? 'border-red-400 bg-red-50' : 'border-gray-300'}`}
              />
            ...
            <div>
              ...
              <input
                ...
                className={`w-full px-3 py-2 border rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 ${errors.email ? 'border-red-400 bg-red-50' : 'border-gray-300'}`}
              />
              ...
              <div className="flex flex-wrap gap-2">
                {COURSE_OPTIONS.map(c => (
                  <button
                    ...
                    className={`text-xs px-3 py-1.5 rounded-full border transition-colors ${form.courses.includes(c) ? 'bg-blue-600 text-white border-blue-600' : 'bg-white text-gray-600 border-gray-300 hover:border-blue-400'}`}
                  >
                    {c}
                  </button>
                ))}
            ...
            <div className={`p-3 rounded-lg border ${errors.lgpd ? 'border-red-300 bg-red-50' : 'border-gray-200 bg-gray-50'}`}>
              <label className="flex items-start gap-3 cursor-pointer">
                <input
                  type="checkbox"
                  checked={form.lgpd}
                  onChange={e => { setForm(p => ({ ...p, lgpd: e.target.checked })); setErrors(p => ({ ...p, lgpd: undefined })); }}
...
interface DeleteModalProps { student: Student; onConfirm: () => void; onClose: () => void }

function DeleteModal({ student, onConfirm, onClose }: DeleteModalProps) {
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      <div className="absolute inset-0 bg-black/30" onClick={onClose} />
...
export default function Students() {
  ...
  const handleSave = (data: { name: string; email: string; courses: string[] }) => {
    if (editTarget) {
    ...
    <div className="p-6 space-y-4">
      ...
      <div className="bg-white rounded-xl border border-gray-200 p-4 flex flex-wrap gap-3 items-center">
        ...
        <div className="flex items-center gap-2">
          ...
          {['todos', 'ativo', 'bloqueado', 'pendente'].map(s => (
            <button key={s} onClick={() => { setStatusFilter(s); setPage(1); }}
              className={`text-xs px-2.5 py-1 rounded-full border transition-colors capitalize ${statusFilter === s ? 'bg-blue-600 text-white border-blue-600' : 'bg-white text-gray-600 border-gray-200 hover:border-blue-400'}`}>
              {s}
            </button>
          ))}
        </div>
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-gray-200 bg-gray-50">
                ...
                {['Aluno', 'E-mail', 'Cursos', 'Status', 'Cadastro', 'Ações'].map(h => (
            ...
            <tbody className="divide-y divide-gray-100">
              {paginated.map(student => (
                <tr key={student.id} className="hover:bg-gray-50 transition-colors cursor-pointer" onClick={() => setDetailStudent(student)}>
                  <td className="pl-4 pr-2 py-3" onClick={e => e.stopPropagation()}>
                    <input type="checkbox" checked={selected.includes(student.id)} onChange={() => toggleSelect(student.id)}
                      className="rounded border-gray-300 text-blue-600 focus:ring-blue-500" />
                  </td>
                  ...
                  <td className="px-3 py-3">
                    <div className="flex flex-wrap gap-1">
                      ...
                      {student.courses.length > 2 && (
                        <span className="text-xs px-2 py-0.5 bg-gray-100 text-gray-500 rounded-full">+{student.courses.length - 2}</span>
                      )}
                    </div>
                  </td>
                  <td className="px-3 py-3">
                  ...
                  <td className="px-3 py-3 pr-4" onClick={e => e.stopPropagation()}>
                    <div className="flex items-center justify-end gap-1">
                      ...
                      <button onClick={() => handleBlock(student)}
                        className={`p-1.5 rounded-lg transition-colors ${student.status === 'bloqueado' ? 'text-amber-500 hover:bg-amber-50' : 'text-gray-400 hover:bg-amber-50 hover:text-amber-600'}`}
                        title={student.status === 'bloqueado' ? 'Desbloquear' : 'Bloquear'}>
                        <Ban size={15} />
                      </button>
            ...
            <div className="flex items-center gap-1">
              ...
              {Array.from({ length: totalPages }, (_, i) => (
                <button key={i + 1} onClick={() => setPage(i + 1)}
                  className={`w-7 h-7 text-xs rounded-lg transition-colors ${page === i + 1 ? 'bg-blue-600 text-white' : 'hover:bg-gray-100 text-gray-600'}`}>
                  {i + 1}
                </button>
              ))}
      ...
      {formOpen && (
        <StudentForm
          initial={editTarget ? { name: editTarget.name, email: editTarget.email, courses: editTarget.courses, lgpd: true } : null}
          onSave={handleSave}
          onClose={closeForm}
        />
      )}
