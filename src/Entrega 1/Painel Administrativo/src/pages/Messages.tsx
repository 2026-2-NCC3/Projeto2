  ...
  return (
    <div className="p-6 space-y-4">
      ...
      <div className="flex border-b border-gray-200">
        {[{ id: 'compose', label: 'Compor mensagem' }, { id: 'history', label: 'Histórico de envios' }].map(t => (
          <button
            key={t.id}
            onClick={() => setTab(t.id as 'compose' | 'history')}
            className={`px-4 py-2.5 text-sm border-b-2 transition-colors ${
              tab === t.id ? 'border-blue-600 text-blue-700 font-medium' : 'border-transparent text-gray-500 hover:text-gray-700'
            }`}
          >
            {t.label}
          </button>
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-gray-200 bg-gray-50">
                {['Assunto', 'Destinatários', 'Enviado em', 'Taxa de leitura'].map(h => (
